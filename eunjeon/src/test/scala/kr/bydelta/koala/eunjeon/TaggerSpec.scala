...
class TaggerSpec extends Specification {
  ...
  "EunjeonTagger" should {
    ...
    "tag a sentence" in {
      ...
      val original = Analyzer.parseEojeol(sent)

      tagged.map(_.map(_.surface).mkString("+")).mkString(" ") must_== original.map(_.nodes.map {
        e =>
          val arr = e.morpheme.feature.last
          if (arr == "*") e.morpheme.surface
          else arr.replaceAll("([^/]+)/[^\\+]+", "$1")
      }.mkString("+")).mkString(" ")
      tagged.map(_.surface).mkString(" ") must_== original.map(_.surface).mkString(" ")
    ...
    "be thread-safe" in {
      ...
      val singlethreaded = sents.map {
        sent =>
          Analyzer.parseEojeol(sent).map(_.nodes.map {
            e =>
              val arr = e.morpheme.feature.last
              if (arr == "*") e.morpheme.surface
              else arr.replaceAll("([^/]+)/[^\\+]+", "$1")
    ...
    "supports dictionary" in {
      ...
      Dictionary.addUserDictionary("아햏햏" -> POS.NNP, "개벽이" -> POS.NNP, "Hwawei" -> POS.NNP)
