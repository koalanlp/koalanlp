...
public class HannanumParser implements Parser {
	/** 한나눔 형태소 분석기 */
	private static volatile HannanumTagger tagger;
	/** 한나눔에서 의존관계 분석에 사용하는 버클리 분석기 */
	private static volatile BerkeleyParserWrapper wrapper;
	...
	public HannanumParser() throws Exception {
		...
		if (tagger == null) {
			tagger = HannanumTagger.getDefaultTagger();
		}
		if (wrapper == null) {
