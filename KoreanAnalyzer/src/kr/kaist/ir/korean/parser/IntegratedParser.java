...
/**
 * 양쪽 방법론 모두의 결과에서 공통된 정보만을 취합. 이 과정에서 어절 분석 구조가 다를 경우, KKMA를 따르도록 한다.
 * 이 과정에서 Root는 없어진다.
 * 
 * @author 김부근
 * @since 2014-08-05
			...
			while (it.hasNext()) {
				...
				for (TaggedWord dep : dependents) {
					...
					if (i != null && i != index && kdep.containsKey(i)) {
						...
						FunctionalTag kkmaTag = TagConverter.getDependencyTag(kkmaRawTag, TaggerType.KKMA);
						if(dep.getTag() == kkmaTag || kkmaRawTag.contains("대상")){
							w.addDependant(value.getWordAt(i), dep.getTag(), dep.getRawTag()+"/"+kkmaRawTag);
						}
					}
				}
			}
