...
trait BasicWordLearnerSpecs[X] {
  ...
  final val EXCLUDED_SET: Seq[String] = Seq("논의하기", "실시하기", "협의하기")
  final val INCLUDED_SET: Seq[String] = Seq("특별법", "새누리당", "새정치민주연합", "운영위원장실", "특검후보추천위원", "유가족", "개정안", "대정부질문", "교섭단체대표연설")
  final val INCLUDED_SET_2: Seq[String] = Seq("특별법", "새누리당", "새정치민주연합", "유가족", "개정안")

  lazy val tagger = getTagger
  lazy val learner = new BasicWordLearner(tagger, getDict)

  lazy final val INC_1 = INCLUDED_SET.partition(word => tagger.tagSentence(word).length > 1)
  lazy final val INC_2 = INCLUDED_SET_2.partition(word => tagger.tagSentence(word).length > 1)
}
