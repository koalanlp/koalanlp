    ...
    return {
        describe("Parser") {
            ...
            it("parses a sentence instance") {
                Examples.exampleSequence().filter { it.first == 1 }.forEach {
                    val parser = getParser()
                    val (intermediate, surface) = parser.convert(it.second)[0]
                    val sentence = parser.convert(intermediate)

                    // Convert 과정에서 기존 tag가 변화될 수 있으므로, 일부러 intermediate type으로 옮겼다가 돌아옴.
                    val directAnalyze = parser.convert(parser.convert(parser.analyze(surface)[0]))
                    val indirectAnalyze = parser.analyze(sentence)
