...
object Examples {
    ...
    private val examples = listOf(
            """01 1+1은 2이고, 3*3은 9이다.
              ...
              |01 대선 출마를 선언한 민주통합당 손학규 상임고문이 5일 오후 서울 세종문화회관 세종홀에서 열린 '저녁이 있는 삶-손학규의 민생경제론'출판기념회에서 행사장으로 들어서며 손을 들어 인사하고 있다.2012.7.5/뉴스1""".trimMargin(),
            ...
            """03 북한이 도발을 멈추지 않으면 미국이 북핵 시설을 타격해도 군사개입을 하지 않겠다. 중국 관영 환구시보가 밝힌 내용인데요. 중국이 여태껏 제시한 북한에 대한 압박 수단 가운데 가장 이례적이고, 수위가 높은 것으로 보입니다.
              ...
              |02 중국 관영매체가 그동안 북한에 자제를 요구한 적은 있지만, 군사지원 의무제공 포기 가능성과 함께 유엔 안보리 제재안을 먼저 제시한 것은 이례적입니다. 미·중 빅딜에 따른 대북압박 공조 가능성이 제기되는 가운데 북한이 어떤 반응을 보일지 관심이 쏠립니다.""".trimMargin(),
            ...
            """01 홍준표 '돼지흥분제 논란' 일파만파…사퇴 요구 잇달아(종합)
              ...
              |01 이들은 "현역 국회의원인 시점에 자서전을 내면서 부끄러운 범죄사실을 버젓이 써놓고 사과 한마디 없다는 것은 더 기막히다"라면서 "대선후보가 아닌 정상적인 사고를 가진 한 인간으로서도 자질부족인 홍 후보의 사퇴를 촉구한다"고 밝혔다.""".trimMargin(),
            ...
            """01 박근혜 전 대통령이 거주하던 서울 삼성동 자택은 홍성열 마리오아울렛 회장(63)이 67억5000만원에 매입한 것으로 확인되었다.
              ...
              |01 한편 박 전 대통령은 최근 삼성동 자택을 매각하고 내곡동에 새 집을 장만한 것으로 확인되었으며 이달 중 내곡동으로 이삿짐을 옮길 것으로 알려졌다.""".trimMargin(),
        ...
        describe("SentenceSplitter") {
            ...
            it("splits sentences") {
                Examples.exampleSequence(requireMultiLine = true).forEach {
                    getSplitter().sentences(it.second).size `should be equal to` originalSplitCounter(it.second)
                }
            }

            it("should be thread-safe") {
                val sents = Examples.exampleSequence(duplicate = 2, requireMultiLine = true)
                ...
                val singleThreaded = sents.flatMap {
                    splitter(it.second)
                }

                multiThreaded.zip(singleThreaded).forEach {
...
fun TaggerSpek(getTagger: () -> CanTag,
    ...
    return {
        describe("Tagger") {
            ...
            it("should be thread-safe") {
                expectThreadSafe(Examples.exampleSequence().filter { it.first == 1 }.map { it.second })
            }
                ...
                isSentenceSplitterImplemented -> {
                    it("matches sentence split spec") {
                        ...
                        Examples.exampleSequence(requireMultiLine = true).forEach {
                            ...
                            splits.size `should be equal to` it.first
                        }
                    }
                }
                ...
                isParagraphImplemented -> {
                    it("tags paragraphs") {
                        ...
                        Examples.exampleSequence().forEach {
                            ...
                            } `should be equal to` orig.joinToString("\n")
                        }
                    }
                }
...
data class Conversion(val tag: String,
                      val toTagger: Boolean = true,
                      val toSejong: Boolean = true)
            ...
            it("converts tags correctly") {
                POS.values().forEach { iPOS ->
                    getMapping(iPOS).forEach {
                        val (tag, toTagger, toSejong) = it

                        if (toTagger)
                            to(iPOS) `should be equal to` tag
                        if (toSejong)
                            from(tag) `should equal` iPOS
                    }
                }

                from(null) `should be` POS.NA
            }
        }
    }
}
...
fun DictSpek(dict: CanCompileDict,
    ...
    return {
        describe("Dictionary") {
            ...
            it("extracts its system dictionary") {
                { dict.getBaseEntries { it.isNoun() } } `should not throw` AnyException
                { dict.getBaseEntries { it.isPredicate() } } `should not throw` AnyException
                { dict.getBaseEntries { it.isModifier() } } `should not throw` AnyException
            ...
            it("should provide way to access user-generated items") {
                val targets = listOf("테팔코리아" to POS.NNP, "구글코리아" to POS.NNP, "오라클코리아" to POS.NNP,
                        "엔비디아" to POS.NNP, "마이크로소프트" to POS.NNP, "구글하" to POS.VV,
                        "대애박" to POS.IC, "대애박" to POS.MM,
                        "김종국" to POS.NNP, "지나" to POS.VV, "AligatorSky" to POS.NNP).filter {
                    importFilter(it.second)
                }.toTypedArray()
                val itemsNotExist = dict.getNotExists(true, *targets)

                itemsNotExist.forEachIndexed { index, pair ->
                    when (index % 4) {
                        0 -> {
                            { dict.addUserDictionary(pair) } `should not throw` AnyException
                        }
                        1 -> {
                            { dict.addUserDictionary(pair.first, pair.second) } `should not throw` AnyException
                        }
                        2 -> {
                            { dict.addUserDictionary(listOf(pair.first), listOf(pair.second)) } `should not throw` AnyException
                        }
                        else -> {
                            { dict += pair } `should not throw` AnyException
                        }
                    }
                }
                ...
                dict.getItems() `should contain all` itemsNotExist
            }

            val dictSample = object : CanCompileDict {
                val words = listOf("마이크로소프트" to POS.NNP,
                        "베지밀" to POS.NNP, "크롬캐스트" to POS.NNP, "하트비트" to POS.NNP).filter {
                    importFilter(it.second)
                }.toMutableSet()
        ...
        describe("Parser") {
            ...
            it("parses a sentence instance") {
                Examples.exampleSequence().filter { it.first == 1 }.forEach {
                    ...
                    directAnalyze `should equal` indirectAnalyze
                }
            }
