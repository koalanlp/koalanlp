...
import kr.bydelta.koala.traits.{CanParse, CanSplitSentence, CanTag}
...
object Implicit {
  ...
  implicit class StringOp(str: String) {
    ...
    /**
      ...
      */
    def toParsed(implicit parser: CanParse): Seq[Sentence] = parser.parse(str)
  ...
  implicit class SentenceOp(sent: Sentence) {
    /**
      ...
      * @return Sentence instance (parsed)
      */
    def toParsed(implicit parser: CanParse): Sentence = parser.parse(sent)
  }
  ...
  implicit class SentenceSeqOp(sent: Seq[Sentence]) {
    /**
      ...
      */
    def toParsed(implicit parser: CanParse): Seq[Sentence] = parser.parse(sent)
  }
