...
import kr.bydelta.koala.traits.{CanCompileDict, CanLearnWord, CanTag, CanTagOnlyAParagraph}
...
/**
  ...
  */
class BasicWordLearnerSpecs extends Specification {
  ...
  """평일 불구하고 속초행 버스 거의 매진
    ...
    |추가 문장. 갤럭시 에스 9가 출시되었다.
    |추가 문장. 갤럭시 에이는 이러한 느낌을.
    | """.stripMargin.split("\n").toSeq

  def getTagger:CanTag = new CanTagOnlyAParagraph[Seq[String]] {
  ...
  def getDict:CanCompileDict = new CanCompileDict {
